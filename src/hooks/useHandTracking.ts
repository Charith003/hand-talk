...
export function useHandTracking(options: HandTrackingOptions = {}) {
  const { confidenceThreshold = DEFAULT_CONF, onPrediction } = options;
  ...
  // Load model + labels (demo-mode fallback if not present)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        if (cancelled) return;
        // Fallback: demo mode with mock vocabulary
        labelsRef.current = [
          "hello", "thank you", "yes", "no", "please",
          "sorry", "help", "water", "food", "love",
        ];
        setDemoMode(true);
        setStatus("Demo mode (no trained model found)");
      } finally {
  ...
  const onResults = useCallback(
    async (results: any) => {
      ...
      if (modelRef.current && !demoMode) {
        ...
        // Demo: pick a pseudo-stable word from keypoint variance
    ...
    [drawSkeleton, demoMode, confidenceThreshold, onPrediction],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (videoRef.current) {
        ...
        try {
          await camera.start();
          setStatus(demoMode ? "Demo mode — camera live" : "Live inference");
        } catch (e) {
